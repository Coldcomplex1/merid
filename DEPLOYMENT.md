...
2. **Give Vercel access to the repo.** On the Vercel dashboard, click **Add New -  - ¶ - Üí Project**.
...
4. **Check the settings.** Vercel auto-detects the build:
   ...
   `VITE_FIREBASE_*` variables (values come from Firebase Console - Üí
   Project settings - Üí Your apps - Üí SDK setup; see `.env.example` for the
...
5. **Wait ~1 minute.** When the confetti screen appears, your site is live. The URL looks like
   ...
   **Continue to Dashboard** - Üí the URL is shown under *Domains*.

6. *(Optional)* **Nicer address:** Project - Üí **Settings - Üí Domains** lets you rename the free
...
1. Firebase Console - Üí **Authentication - Üí Sign-in method** - Üí enable **Email/Password**.
2. **Firestore Database - Üí Create database** (Production mode).
3. Firestore - Üí **Rules** tab - Üí paste the contents of `firestore.rules` from this
   repo - Üí **Publish**. (Nothing works until the rules are published -  -  - the
   database starts locked.)
4. **Authentication - Üí Settings - Üí Authorized domains** - Üí add every domain that
   serves the site (your `*.vercel.app` URL, `merid.site`, plus `localhost` for
   dev). Sign-in is blocked from any domain not on this list.
5. Vercel - Üí Environment Variables - Üí the six `VITE_FIREBASE_*` values - Üí Redeploy.
...
### Checking a deploy
The Vercel dashboard - Üí your project - Üí **Deployments** shows every publish with a
 - úÖ Ready / - ùå Error status and full build logs. If a deploy fails, the live site keeps
